...
| 4  | Update forgelore:build tool to read model from agent file | done | plugin | `readModelFromAgent(directory, "forgelore-builder.md", fallback)` reads `.opencode/agents/forgelore-builder.md`, extracts `model:` from frontmatter via regex. Falls back to `anthropic/claude-sonnet-4-20250514` if file missing. `args.model` override still takes priority. |
| 5  | Update forgelore:validate tool to read model from agent file | done | plugin | Same pattern as Task 4 but reads `forgelore-validator.md`. Falls back to `anthropic/claude-sonnet-4-20250514`. |
