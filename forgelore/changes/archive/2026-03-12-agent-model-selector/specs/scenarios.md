...
1. **User selects models interactively**: User runs `bunx @forgelore/opencode`. After plugin config and CLI install, they see 4 prompts — one per role. Each shows the role name, description, and a default. User picks models from a list or types a custom one. Agent files are written with chosen models.

2. **User accepts all defaults**: User runs `bunx @forgelore/opencode` and presses Enter through all 4 model prompts. Defaults are used (e.g. Sonnet for builder, Opus for planner). Agent files match defaults.
...
4. **Re-running setup**: User runs `bunx @forgelore/opencode` again on a project that already has agent files. Existing agent files are overwritten with new model selections (current behavior for all setup steps).
